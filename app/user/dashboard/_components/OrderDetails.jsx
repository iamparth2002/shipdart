...
import { ArrowLeft, Package, User, MapPin, Phone, RotateCcw, Truck, DollarSign, Box } from 'lucide-react'
...
export default function OrderDetails({ order, index, setSelectedOrderIndex }) {
  ...
  const formatFieldName = (field) => {
    return field.split('_').map(word => word.charAt(0) + word.slice(1).toLowerCase()).join(' ')
  }
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {section.fields.map((field) => (
                  <div key={field} className="flex flex-col">
                    ...
                    <span className="font-medium">{order[field] !== undefined ? formatFieldValue(field, order[field]) : 'N/A'}</span>
                  </div>
                ))}
              </div>
